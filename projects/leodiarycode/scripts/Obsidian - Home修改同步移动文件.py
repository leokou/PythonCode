...
from obsidian_common import (
    VAULT_ROOT, SKIP_EXTENSIONS, SKIP_FILES_PREFIX, SKIP_FILES_EXACT,
    should_skip_dir, ensure_title_header, strip_frontmatter,
)
...
def _extract_link_annotations(content: str) -> dict:
    """从内容中提取 [[链接]] 后面的用户注释，返回 {链接名: 注释文本} 字典。"""
    annotations = {}
    for line in content.splitlines():
        ...
        if match:
            ...
            if user_text:
                ...
                annotations[pure_name] = user_text
    return annotations


def _merge_link_annotations(new_content: str, old_content: str) -> str:
    """合并用户注释：以旧🧩文件中的注释为准，将其应用到新内容中。
    如果新内容中某链接没有注释，用旧的补上；如果两边都有注释，保留旧的。"""
    old_annotations = _extract_link_annotations(old_content)
    ...
    for line in lines:
        ...
        if match:
            ...
            pure_name = link_name.replace('\\', '/').split('/')[-1]
            if pure_name in old_annotations:
                # 旧文件中有注释，以旧的为准（用户手动维护的）
                old_anno = old_annotations[pure_name]
                if existing_text != old_anno:
                    line = re.sub(r'(\[\[[^\]]+\]\])(.*)', 
                                  lambda m: m.group(1) + ' ' + old_anno, 
                                  line.rstrip('\r\n')) + '\n'
        result.append(line)
    return ''.join(result)
...
def sync_home_to_chips(base_dir: Path) -> int:
    """将 home 文件中各模块的内容反向同步回 🧩 目录文件。保留用户在 [[链接]] 后加的注释。"""
    ...
    for stem, section_content in sections.items():
        ...
        try:
            ...
            # 合并注释：以🧩中用户手动维护的注释为准
            new_content = _merge_link_annotations(new_content, old_content)
...
def main():
    # 安全检查：防止健康检查脚本无参数运行时误触发搬移
    if len(sys.argv) > 1 and sys.argv[1] in ("--help", "--check", "--version"):
        print(f"用法：{sys.argv[0]} [--check]")
        print("  --check  仅检查状态，不执行搬移")
        print("  无参数    执行 Home 驱动整理（需确保 vault 索引已完成）")
        sys.exit(0)
    ...
    for dir_file in dir_files:
        ...
        # 检测是否为根目录的目录文件（如 🧩 目录-1- 🤖AI 相关.md）
        # 如果是根目录的目录文件，跳过文件移动操作，只处理子目录中的目录文件
        # 防止把所有子文件夹的文件都移到根目录
        # 注意：不能用文件名匹配判断（如 `🧩 目录-{parent.name}`），因为目录名可能含前缀（如 `7- `）
        # 而文件名不含，导致保护失效。只用物理层级判断。
        is_root_chip = (dir_file.parent.parent == VAULT_ROOT)
        ...
        for link_name in links:
            ...
            if is_root_chip:
                stats["already_in_place"] += 1
                log.info(f"   ⏭️  [[{link_name}]] 已在子目录中（根目录目录文件跳过移动）")
                continue

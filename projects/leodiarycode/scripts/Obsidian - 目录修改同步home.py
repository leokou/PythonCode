...
from obsidian_common import (
    ...
    should_skip_dir, ensure_title_header, strip_frontmatter, read_text_safe,
)
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ("--help", "--check", "--version"):
        ...
        print("  无参数    执行 目录驱动整理（需确保 vault 索引已完成）")
        sys.exit(0)
    ...
    for dir_file in dir_files:
        ...
        for link_name in links:
            ...
            # 一级目录的 🧩 目录文件跳过移动，防止把子文件提到根目录
            if dir_file.parent.parent == VAULT_ROOT:
                stats["already_in_place"] += 1

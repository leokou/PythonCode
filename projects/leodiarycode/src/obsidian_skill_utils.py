...
def cmd_update_index_entry(index_file: str, old_link: str, new_link: str, summary: str) -> None:
    ...
    new_lines = []
    for line in lines:
        # 删除旧链接行（仅当 old_link 非空且与 new_link 不同时才移除，避免摘要更新时误删）
        if old_link and old_link != new_link and f'[[{old_link}]]' in line:
            updated = True
            continue
        new_lines.append(line)

    # 📋 文件清单 模块已废弃（与 📂 目录结构树 重复），不再写入该区域。
    # 新链接的 ✍️ 摘要改写到「## 📂 目录结构树」区域中对应的 [[new_link]] 行。
    ...
    for line in new_lines:
        ...
        if in_tree and line.startswith('## '):
            ...
            final_lines.append(line)
            continue
        if in_tree and f'[[{new_link}]]' in line and summary and '✍️' not in line:
            # 在已有链接行补摘要，避免重复插入新条目
    ...
    if not inserted:
        # 目录结构树中未找到该链接：说明尚未被 index_updater.py 全量重建纳入，
        # 不写孤立条目，待下次重建自动同步。
        print(f"⚠️ 索引 {index_file} 的目录结构树中未找到 [[{new_link}]]，"
              f"摘要将在 index_updater.py 重建时同步；跳过写入。")
        index_path.write_text(''.join(new_lines), encoding='utf-8')
    else:
        index_path.write_text(''.join(final_lines), encoding='utf-8')
        print(f"✅ 已更新索引：{index_file}（删除旧链接，目录结构树补摘要）")
...
def cmd_verify_move(src_path: str, dst_path: str, index_file: str = "", new_link: str = "") -> None:
    ...
    # AI_INDEX 同步检查 (v3.0: 检查 AI_INDEX.json 二元组 [path, type])
    ai_index_ok = None
    if dst.exists():
        try:
            vault_root = Path(r"D:\Obsidian\LeoDiary")
            index_json = vault_root / ".ai-index" / "AI_INDEX.json"
            if index_json.exists():
                import json as _json
                index_data = _json.loads(read_text_safe(index_json))
                # AI_INDEX.json 是 [[path, type], ...] 列表
                rel_path = str(dst.relative_to(vault_root)).replace("\\", "/")
                found = any(isinstance(e, list) and len(e) >= 1 and e[0] == rel_path for e in index_data)
                if found:
                    ai_index_ok = True
                else:
                    ai_index_ok = False
                    issues.append(f"AI_INDEX 中无此文件，请运行 tools/rebuild-ai-index.py（文件：{rel_path}）")
            else:
                issues.append(".ai-index/AI_INDEX.json 不存在，请运行 tools/rebuild-ai-index.py")
        except Exception as e:
...
VALID_KNOWLEDGE_DIRS = {
    ...
    "5- 🧁项目", "6- 🎬影视", "leo settings/🧠思维框架", "leo settings/📜核心规则",
}
...
def _check_doc_consistency(vault: Path, skills_dir: Path, python_dir: Path) -> list:
    ...
    # 检查知识处理系统使用手册
    manual_path = vault / "skills" / "知识处理系统使用手册3.0.md"
...
def _check_skill_doc_consistency(vault: Path, skills_dir: Path) -> list:
    ...
    for df in doc_files:
        ...
        if name.startswith("🧩") or name.startswith("📖") or name.startswith("🏠"):
            continue
        if name in ("知识处理系统使用手册3.0.md", "知识处理系统使用手册1.0.md", "obsidian_skill_utils.py 说明.md",
...
def _check_doc_content_drift(vault: Path, skills_dir: Path) -> list:
    ...
    docs_to_check = [
        ...
        ("知识处理系统使用手册3.0.md", vault / "skills" / "知识处理系统使用手册3.0.md"),
...
def _check_file_naming_convention(vault: Path) -> list:
    ...
    # 非 skill 说明文件（跳过命名检查）
    skip_files = {
        "知识处理系统使用手册3.0.md", "知识处理系统使用手册1.0.md", "🧩 目录-skills.md", "🏠 home-skills.md",
...
def _extract_doc_numbers(content: str, doc_name: str) -> dict:
    """从文档内容中提取关键数字"""
    info = {"doc": doc_name}
    m = re.search(r'(\d+)\s*个.*?[Ss]kill', content)
    if m:
...
def _check_cross_doc_consistency(vault: Path) -> list:
    ...
    docs = {
        "CLAUDE.md": vault / "CLAUDE.md",
        "README.md": vault / "README.md",
        "使用手册": vault / "skills" / "知识处理系统使用手册3.0.md",
    }
...
def _check_ai_index_layer(vault: Path, skills_dir: Path, python_dir: Path) -> list:
    """检查 AI 检索加速层（.ai-index）完整性"""
    results = []
    category = "AI检索加速层(.ai-index)"

    # A. .ai-index/ 关键文件完整性（v3.0）
    ai_index_dir = vault / ".ai-index"
    
    key_files = {
        "AI_INDEX.json": "核心导航索引（二元组 [path, type]）",
        "metadata.json": "索引元数据（版本/生成时间/文件数）",
        "query-cache.json": "查询缓存",
    }
    for fname, desc in key_files.items():
        fpath = ai_index_dir / fname
        if fpath.exists() and fpath.stat().st_size > 0:
            results.append(_make_result(category, f"{fname}", "pass",
                                       f"{fname} 存在（{desc}）"))
        else:
            results.append(_make_result(category, f"{fname}", "fail",
                                       f"{fname} 不存在或为空",
                                       "文件存在", "运行 tools/rebuild-ai-index.py"))

    # 检查领域索引文件（domain-*.json）
    domain_files = list(ai_index_dir.glob("domain-*.json"))
    if len(domain_files) >= 10:
        results.append(_make_result(category, "领域索引文件", "pass",
                                   f"{len(domain_files)} 个领域索引文件存在"))
    else:
        results.append(_make_result(category, "领域索引文件", "warn",
                                   f"仅 {len(domain_files)} 个领域索引文件（期望 ≥10）",
                                   "足够领域文件", "运行 tools/rebuild-ai-index.py"))

    # 检查 metadata.json 内容
    meta_path = ai_index_dir / "metadata.json"
    if meta_path.exists():
        try:
            import json as _json
            ...
            if version == "3.0":
                results.append(_make_result(category, "metadata.json 版本", "pass",
                                           f"v{version}，{file_count} 文件"))
            else:
                results.append(_make_result(category, "metadata.json 版本", "warn",
                                           f"版本 {version}（期望 3.0）"))
        except Exception:
            results.append(_make_result(category, "metadata.json 内容", "warn",
                                       "JSON 解析失败"))

    # B. tools/rebuild-ai-index.py 检查（v3.0 唯一索引生成工具）
    tools_script = vault / "tools" / "rebuild-ai-index.py"
    if tools_script.exists():
        results.append(_make_result(category, "tools/rebuild-ai-index.py", "pass",
                                   "索引重建脚本存在"))
        content = tools_script.read_text(encoding='utf-8')

        # 关键函数检查
        key_funcs = ["scan_markdown_files", "build_ai_index", "build_domain_index"]
        for func in key_funcs:
            if f"def {func}" in content:
                results.append(_make_result(category, f"函数 {func}", "pass",
                                           f"{func} 存在"))
            else:
                results.append(_make_result(category, f"函数 {func}", "fail",
                                           f"{func} 缺失",
                                           "函数存在", "检查 rebuild-ai-index.py"))

        # --status 参数检查
        if "--status" in content or "status" in content:
            results.append(_make_result(category, "--status 参数", "pass",
                                       "状态查询参数存在"))
        else:
            results.append(_make_result(category, "--status 参数", "warn",
                                       "缺少 --status 参数"))
    else:
        results.append(_make_result(category, "tools/rebuild-ai-index.py", "fail",
                                   "索引重建脚本不存在",
                                   "脚本存在", "检查 tools/ 目录"))
    ...
    skills_to_check = {
        ...
        "obsidian-knowledge-organizer": {
            "keywords": ["rebuild-ai-index.py", "incremental"],
            "desc": "Organizer 触发 rebuild-ai-index.py 增量更新"
        },
        "obsidian-pipeline": {
            "keywords": ["rebuild-ai-index.py"],
            "desc": "Pipeline 触发 rebuild-ai-index.py 索引更新"
        },
        "obsidian-fire-rename": {
            "keywords": ["rebuild-ai-index.py", "incremental"],
            "desc": "Fire-rename 触发 rebuild-ai-index.py 索引更新"
        },
    }
            ...
            if files_count > 0 and actual_files > 0:
                if abs(files_count - actual_files) <= 5:
                    ...
                    results.append(_make_result(category, "索引漂移", "warn",
                                               ...
                                               action="运行 tools/rebuild-ai-index.py"))
        except Exception as e:

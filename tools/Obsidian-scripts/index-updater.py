...
# 自动生成的聚合文件（📦 inbox / 🧠 FlashNote），全局跳过，不进入任何索引
SKIP_GENERATED_FILES = {"📦 inbox", "🧠 FlashNote"}
...
def scan_md_files(dir_path: Path, index_filename: str) -> Set[str]:
    """扫描目录下所有直接 .md 文件（排除索引文件自身和自动生成的聚合文件），返回文件名（不含扩展名）集合"""
    md_files = set()
    try:
        for entry in dir_path.iterdir():
            if entry.is_file() and entry.suffix.lower() == '.md':
                if entry.name != index_filename and entry.stem not in SKIP_GENERATED_FILES:
    ...
    for root, dirs, files in os.walk(sub_dir):
        ...
        for f in sorted(files):
            ...
            if should_skip_file(f):
                continue
            file_stem = f[:-3]
            # 过滤自动生成的聚合文件（📦 inbox / 🧠 FlashNote）
            if file_stem in SKIP_GENERATED_FILES:
                continue
            file_count += 1
            indent = "  " * (depth + 1)
            file_stem = f[:-3]
...
def generate_domain_index(target_dir: Path, first_dir: Path,
    ...
    """
    收集单个一级目录的领域元数据（不再生成 📖目录 索引-{目录名}.md 二级索引文件，
    领域导航统一指向该目录内的 🏠 home-{目录名}.md）。
    返回该领域的元数据：{name, file_count, dir_count, home_link, keywords, sample_files}
    """
    if freq_data is None:
        freq_data = {}

    dir_name = first_dir.name
    home_link = f"{dir_name}\\🏠 home-{dir_name}"
...
def generate_directory_index(target_dir: Path) -> int:
    """
    生成总路由索引：📖目录 索引.md（含关键词快速定位 + 领域导航）。
    不再生成各领域二级索引文件（📖目录 索引-{目录名}.md），领域导航链接指向各一级目录的 home 文件。

    最快检索路径：关键词快速定位表 → 直接命中文件
                 → 未命中则按领域导航 → 打开对应目录的 home 文件
    """
    ...
    for first_dir in first_level_dirs:
        ...
        print(f"  📄 {meta['name']} : {meta['file_count']} 个文件")
    
    # 清理遗留的旧版二级索引文件（📖目录 索引-*.md，不再生成）
    for f in target_dir.iterdir():
        if f.is_file() and f.name.startswith("📖目录 索引-") and f.name.endswith(".md"):
            f.unlink()
            print(f"  🗑️ 删除旧版二级索引：{f.name}")
    ...
    lines.append("## 🧭 领域导航\n\n")
    lines.append("按一级目录划分的领域导航，打开对应目录的 home 文件继续深入：\n\n")
    lines.append("| 领域 | 文件数 | 核心关键词 |\n")
    lines.append("|------|-------|----------|\n")
    
    for meta in domain_metas:
        kws = "、".join(meta['keywords'][:8])
        lines.append(f"| [[{meta['home_link']}]] | {meta['file_count']} | {kws} |\n")
    
    lines.append("\n> 💡 根据问题关键词匹配上表，**只打开命中的领域 home 文件**，其他跳过。\n\n")
    lines.append("---\n\n")
...
def main():
    ...
    print("-" * 60)
    print("📖 生成总路由索引（含领域导航，指向 home 文件）...")

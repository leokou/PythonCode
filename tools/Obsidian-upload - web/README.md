...
- **实时预览**：右侧 marked.js 渲染，与编辑区**按比例双向同步滚动**（编辑区滚动 → 预览定位，预览点击 → 编辑区跳转）
...
- **多标签页**：每个窗口独立多个页签，首行标题自动命名、自动重命名文件、滚轮切换；页签过多时自动折叠为「▼」下拉清单（按页签顺序列出全部隐藏页签，点击一键切换）
- **自动保存防丢失**：编辑变化 2 秒 debounce 覆盖写入 Tab 文件 + 每 30 秒保险保存 + 退出程序后台保存全部页签，**不丢内容**
...
- **设置窗口**：右上角「⚙️」修改默认保存地址，立即生效、重启保持
- **工具箱**：右上角「🛠️」插件式工具窗口（当前内置「删除空行」）
...
| 窗口 | 快捷键 | 保存目标 |
|------|--------|----------|
| 📥 Inbox Capture | `Alt+S` | 保存 Inbox → `D:\Obsidian\LeoDiary\My-Inbox.md` |
| 🧠 FlashNote Capture | `Alt+E` | 保存 FlashNote → `D:\Obsidian\LeoDiary\🧠 FlashNote.md` |
| 📅 Daily Log | `Alt+J` | 保存日志 → `log_dir\yyyy-MM-dd 周X.md`（按日期自动命名，追加不覆盖） |
...
- **📥 工作区**：左侧资源管理器栏，默认隐藏，点左上角窗口名称（如 `📥 My-Inbox`）显示/收起，宽度可拖拽（160-400px）；支持名称/时间排序、展开/折叠全部
- **📑 目录**：解析当前文档标题生成章节大纲，点击跳转编辑区与预览区，光标移动高亮当前章节
- **🕘 历史**：最近文件记录面板，见下节
- 点击「目录 / 历史」按钮切换第三栏内容，再次点击收起/展开；当前选择（`pane_mode`）、各栏宽度、工作区显隐（`workspace_visible`）、宽度（`workspace_width`）与资源树排序（`explorer_sort`）写入 `layout` 配置，重启保持
...
| 文件 | 作用 |
|------|------|
| `settings.json` | 默认保存地址（设置窗口修改，立即生效、重启保持） |
| `pages.json` | 页面元数据（Tab 独立文件的 id / 标题 / 路径） |
| `workspace.json` | 工作区文件夹列表 |
| `layout.json` | 三栏布局（宽度比例 / 目录可见性 / 第三栏模式 / 工作区显隐与宽度），优先写入 EXE 旁 config.json |
...
- 工具列表与排序存于 `tools\tools.json`
- 当前内置：**🧹 删除空行**（清理 Markdown 连续空行）

```
tools/
├── tools.json             工具列表（id/名称/描述/图标/排序）
└── clean_empty_lines/
    ├── config.json
    └── index.js
```
...
4. 内容自动保存到 Tab 文件；点「保存」额外追加到聚合文件
5. 查看「🕘 历史」按时间找回最近编辑的文件，点击重新打开
6. 点窗口 X 隐藏到托盘，随时再次呼出；托盘菜单可退出程序
...
```
Obsidian-upload/
├── main.py                程序入口：pywebview 四窗口 + js_api + 托盘 + 单实例 + 看门狗
├── pages.py               页面元数据 pages.json + Tab 独立文件管理（覆盖写/重命名/恢复）
...
├── layout_store.py        布局记忆（宽度比例 / 目录可见性 / pane_mode / 工作区显隐与宽度 / 资源树排序）
...
├── make_icon.py           生成 app.ico（多尺寸）
├── config.json            配置文件（PicGo / 路径 / 布局 / 文件关联）
...
│   └── __init__.py
├── tools/                 工具插件目录（tools.json + clean_empty_lines/）
├── web/
│   ├── editor.html        编辑窗口结构（工作区 + 三栏 + 工具栏）
│   ├── script.js          CodeMirror / Tab / 图片粘贴 / 保存 / 历史重开 / 工作区打开
...
`config.json` 内嵌于 EXE，复制到 EXE 同目录即可自定义（无需重新打包）：
...
| `layout` | 布局：宽度比例 / 目录可见性 / 第三栏模式（`outline` 目录 / `history` 历史）/ 工作区显隐与宽度 / 资源树排序（`explorer_sort`：`name` 名称 / `time` 时间） |
...
pyinstaller --noconfirm --clean --onefile --windowed ^
  ...
  --hidden-import=storage --hidden-import=pages --hidden-import=settings ^
  --hidden-import=file_assoc --hidden-import=window_manager ^
  --hidden-import=layout_store --hidden-import=history ^
  --hidden-import=workspace --hidden-import=file_tree --hidden-import=search_engine ^
  --hidden-import=capture ^
  --add-data "web;web" --add-data "tools;tools" ^
  --add-data "config.json;." --add-data "commands;commands" --add-data "app.ico;." ^
  main.py
```
...
- **语法检查**：`python -m py_compile main.py capture.py history.py file_ops.py layout_store.py workspace.py file_tree.py file_explorer.py search_engine.py` / `node --check web/*.js web/js/*.js web/script.js`
- **前端模块职责**：`web/js/layout.js` 管布局模式与保存，`resize.js` 管拖拽，`outline.js` 管目录，`history.js` 管历史面板，`explorer.js` 管树渲染，`context-menu.js` 管右键菜单/重命名/删除确认，`workspace.js` 管工作区，`search.js` 管搜索；`script.js` 只做编排与胶水
- **历史持久化**：`history.py` 不依赖 UI / 网络，可独立测试；后端只记录「打开 / 编辑」，分组展示纯前端完成
- **工作区与搜索**：`workspace.py / file_tree.py / file_explorer.py / search_engine.py / file_ops.py` 均为纯后端模块，不依赖 UI，可独立测试；树懒加载逐层扫描、搜索逐行增量匹配不整文件载入内存（搜索范围 = `search_exts` ∪ `explorer_exts`）；文件·文件夹右键操作走 `file_ops.py`（删除绝不物理删除，走回收站；重命名/移动/删除同步 `history` 的 rename / move_path / remove_tree）
- **新增功能原则**：独立模块、可插拔、低耦合；优先新增文件，避免在大文件里堆业务逻辑（详见 `CLAUDE.md`）

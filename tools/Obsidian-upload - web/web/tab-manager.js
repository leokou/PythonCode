...
window.TabManager = (() => {
  ...
  function tabHTML(tab, activeId) {
    ...
    return (
      ...
      `<button class="tab-close" title="关闭页签">×</button>`
    );
  }
  ...
  function setTabStatus(el, status) {
    ...
    badge.className = "tab-status " + status;
    badge.title = text;
  ...
  function renderTabs(listEl, addBtnEl, tabs, activeId, onClick) {
  ...
  /* ---- 关闭确认弹窗（左删除红 / 右保存绿） ---- */
  function confirmClose(tabName, onDelete, onSave) {
    openModal(
      `<div class="modal-title">是否保存当前内容？</div>` +
      `<div class="modal-msg">页签「${escapeHTML(tabName || "未命名")}」即将关闭。</div>` +

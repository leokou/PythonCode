...
一组本地备份脚本，用于自动备份 Claude Skills 与 Obsidian 笔记，防止误删/丢失。

## 包含脚本

| 脚本 | 功能 | 备份目标 | 保留份数 |
|------|------|----------|----------|
| `claude-skill-backup.py` | 备份全部 Claude Skills | `D:\project\skill备份` | 最多 10 份 |
| `leodiary-backup.py` | 备份 Obsidian LeoDiary 笔记 | `D:\Obsidian\Backup` | 最多 20 份 |

## 功能说明

- **claude-skill-backup.py**
  - 源目录：`C:\Users\leokou\.claude\skills`
  - 每次备份生成带时间戳目录（可加备注），自动清理最旧备份
  - 备份过程记录日志到 `skill-backup.log`

- **leodiary-backup.py**
  - 源目录：`D:\Obsidian\LeoDiary`
  - 排除图片/视频/压缩包等大文件（`png/jpg/mp4/zip/pdf` 等）与 `.obsidian`、`.git` 等目录
  - 自动清理最旧备份

## 使用方式

```bash
python claude-skill-backup.py [--remark "备注"]
python leodiary-backup.py [--remark "备注"]
```

示例：

```bash
python claude-skill-backup.py --remark "添加新Skill后"

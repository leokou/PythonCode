@echo off
REM ---------------------------------------------------------------------------
REM chrome-go - build exe
REM   Targets (this dir, tools\chrome-go):
REM     - ChromeGo node downloader (.py)  -> chrome-go.exe
...
echo ===================================================================
echo [BUILD] chrome-go.exe  (%SRC%)
echo ===================================================================
"%PY%" -m PyInstaller --onefile --noconfirm --clean --distpath "%DIST%" --name chrome-go "%SRC%"
if errorlevel 1 (
    echo [FAIL] chrome-go build failed
    pause
    exit /b 1
)
...
echo.
echo [DONE] Generated:
if exist "%DIST%\chrome-go.exe" echo   - %DIST%\chrome-go.exe
endlocal
pause
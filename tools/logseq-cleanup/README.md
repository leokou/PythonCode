...
扫描 Logseq 图谱的 `journals` / `pages` 笔记，找出 `assets` 目录中**未被任何笔记引用**的附件文件，统一移动到 `to_delete` 目录，等待人工确认。

## 功能说明

- 遍历日志（journals）与页面（pages）中的 `.md` 文件，收集所有被引用的附件名
- 对比 `assets` 目录，找出未引用的附件（排除 `.edn` 文件）
- 将无用附件**移动**（非删除）到 `to_delete` 目录，安全可回滚
- 控制台打印每个被清理的文件名

## 使用方式

```bash
python "Logseq - 附件清理脚本 @ 清理无用文件.py"
```

## 注意事项

- **图谱路径为硬编码**：脚本顶部 `os.chdir(...)` 指向本地 Logseq 图谱（iCloudDrive），如路径变更请修改脚本

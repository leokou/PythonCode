@echo off
REM ---------------------------------------------------------------------------
REM logseq-cleanup - build exe
REM   Targets (this dir, tools\logseq-cleanup):
REM     - Logseq attachment cleanup (.py, Chinese filename)  -> logseq-cleanup.exe
...
echo ===================================================================
echo [BUILD] logseq-cleanup.exe  (%SRC%)
echo ===================================================================
"%PY%" -m PyInstaller --onefile --noconfirm --clean --distpath "%DIST%" --name logseq-cleanup "%SRC%"
if errorlevel 1 (
    echo [FAIL] logseq-cleanup build failed
    pause
    exit /b 1
)
...
echo.
echo [DONE] Generated:
if exist "%DIST%\logseq-cleanup.exe" echo   - %DIST%\logseq-cleanup.exe
endlocal
pause
# GitHub 同步工具集 (sync-GitHub)

Claude Skills / Python 代码的 **GitHub 同步** 与 **本地备份** 工具集。

## 包含脚本

| 脚本 | 功能 | 目标 |
|------|------|------|
| `skill-sync-GitHub.py` | Claude Skills → GitHub 同步（clone-同步-commit-push） | `github.com/leokou/leodiary-skills` |
| `skill-sync-agentcode.py` | Skill 同步到其他 Agent（codex / trae / workbuddy / codebuddy 等） | 本机各 Agent skills 目录 + `D:\project` |
| `python-code-sync-GitHub.py` | Python 代码备份 → GitHub | `github.com/leokou/PythonCode` |
| `python-local-backup.py` | Python 代码本地备份 | `D:\project\python备份` |

## 功能说明

- **skill-sync-GitHub.py**
  - 源目录：`C:\Users\leokou\.claude\skills\Obsidian`
  - 每次全量重建 clone → 复制全部 Skill → commit → push 到 `main` 分支
  - 跳过 `.system`、`.obsidian`、`__pycache__` 等目录，附带复制顶层 `README.md`、`CLAUDE.md`
  - 无变更时自动跳过（不产生空提交）

- **skill-sync-agentcode.py**
  - 将 Claude Skills 同步到 Codex / Trae / WorkBuddy / CodeBuddy 等 Agent 的 skills 目录
  - 自动识别嵌套 SKILL.md（如 Agent-Reach），记录日志到 `skill-sync.log`

- **python-code-sync-GitHub.py**
  - 检测 `D:\Python` 的 git 变更 → add → commit → push
  - 无变更时自动跳过

- **python-local-backup.py**
  - 本地整目录备份 `D:\Python`，排除 `__pycache__`、`.git`、`node_modules`、`dist`、`build`
  - 自动清理最旧备份（保留 10 份）
...
```bash
# Skill 同步到 GitHub
python skill-sync-GitHub.py [--message "版本说明"]

# Skill 同步到其他 Agent
python skill-sync-agentcode.py

# Python 代码备份到 GitHub
python python-code-sync-GitHub.py [--message "版本说明"]

# Python 代码本地备份
python python-local-backup.py
```
